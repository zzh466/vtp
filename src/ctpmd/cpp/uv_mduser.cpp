...
const char *uv_mduser::GetTradingDay()
{
...
void uv_mduser::Connect(UVConnectField* pConnectField, void (*callback)(int, void *), int uuid) {
	...
	this->invoke(_pConnectField, T_CONNECT_RE, callback, uuid);//pConnectField�����ⲿ����
}


void uv_mduser::Disconnect()
{
	...
	logger_cout("uv_mduser Disconnect------>object destroyed");
}
...
void uv_mduser::SubscribeMarketData(char *ppInstrumentID[], int nCount, void (*callback)(int, void *), int uuid) {
	...
	for (int i = 0; i < nCount; i++) {
		size_t len = strlen(ppInstrumentID[i]);
		_ppInstrumentID[i] = new char[len + 1];
		memcpy(_ppInstrumentID[i], ppInstrumentID[i], len + 1);
	}

	this->invoke(_ppInstrumentID, T_SUBSCRIBE_MARKET_DATA_RE, callback, uuid);
}
...
void uv_mduser::UnSubscribeMarketData(char *ppInstrumentID[], int nCount, void (*callback)(int, void *), int uuid) {
	...
	this->invoke(_ppInstrumentID, T_UNSUBSCRIBE_MARKET_DATA_RE, callback, uuid);
}
...
void uv_mduser::OnFrontDisconnected(int nReason) {
	...
	CbRtnField* field = new CbRtnField();//������Ϻ���Ҫ����
	field->eFlag = T_ON_DISCONNECT;//FrontConnected
	field->nReason = nReason;
	field->work.data = field;//�������ٺ�ָ�����
	uv_queue_work(uv_default_loop(), &field->work, _on_async, _on_completed);
}
...
void uv_mduser::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *pDepthMarketData) {
	std::string logx = "jason uv_mduser OnRtnDepthMarketData";
	logger_cout(logx.c_str());
...
void uv_mduser::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
	std::string logx = "uv_mduser OnRspError";
	logger_cout(logx.c_str());
...
void uv_mduser::_async(uv_work_t *work) {
	...
	uv_mduser *uv_mduser_obj = static_cast<uv_mduser *>(baton->uv_mduser_obj);
	...
	switch (baton->fun) {
		case T_CONNECT_RE:
		{
			...
			uv_mduser_obj->m_pApi->RegisterSpi(uv_mduser_obj);			// ע���¼���
...
void uv_mduser::invoke(void *field, int ret, void (*callback)(int, void *), int uuid) {
	LookupCtpApiBaton *baton = new LookupCtpApiBaton();
	baton->work.data = baton;
	baton->uv_mduser_obj = this;
